...
def admin_home(request):
    ...
    total_hods = Hod.objects.all().count()
...
def manage_hod(request):
    allHods = User.objects.filter(user_type=2)
    
    for hod in allHods:
        print(hod,hod.id)
    context = {
        'allHods': allHods,
        'page_title': 'Manage HOD'
    }
...
def edit_hod(request, hod_id):
    hod = get_object_or_404(Hod, admin_id=hod_id)
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            try:
                user = User.objects.get(id=hod.admin.id)
                ...
                return redirect(reverse('edit_hod', args=[hod_id]))
            except Exception as e:
        ...
        hod = Hod.objects.get(admin_id=user.id)
...
def delete_hod(request, hod_id):
    hod = get_object_or_404(User, hod__id=hod_id)
    hod.delete()
    messages.success(request, "HOD deleted successfully!")
    return redirect(reverse('manage_hod'))
