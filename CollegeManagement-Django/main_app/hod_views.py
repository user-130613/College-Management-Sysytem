...
def manage_staff(request):
    allStaff = User.objects.filter(user_type=3)
    context = {
        'allStaff': allStaff,
...
def edit_staff(request, staff_id):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            try:
                user = User.objects.get(id=staff.hod.id)
...
def edit_student(request, student_id):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            passport = request.FILES.get('profile_pic') or None
            try:
                user = User.objects.get(id=student.hod.id)
                if passport != None:
                    fs = FileSystemStorage()
                    filename = fs.save(passport.name, passport)
                    passport_url = fs.url(filename)
...
def hod_view_attendance(request):
    ...
    return render(request, "hod_template/hod_view_attendance.html", context)


@csrf_exempt
def get_hod_attendance(request):
...
def hod_view_profile(request):
    hod = get_object_or_404(Hod, hod=request.user)
    ...
    return render(request, "hod_template/hod_view_profile.html", context)


def hod_notify_staff(request):
    staff = User.objects.filter(user_type=3)
    context = {
...
def hod_notify_student(request):
    student = User.objects.filter(user_type=4)
    context = {
...
def send_student_notification(request):
    ...
    student = get_object_or_404(Student, hod_id=id)
    try:
...
def send_staff_notification(request):
    id = request.POST.get('id')
    message = request.POST.get('message')
    staff = get_object_or_404(Staff, hod_id=id)
    try:
        url = "https://fcm.googleapis.com/fcm/send"
        body = {
            'notification': {
...
#        student_name_list.append(student.hod.first_name)
